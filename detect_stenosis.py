...
STATUS_TRACHEO = 1       # 气管阶段。在咽喉部以下，分出支气管之前。特征：圆形度较高，tracheo图像只有一个圆孔
STATUS_SWITCH = 2        # 气管转支气管阶段。特征：tracheo图像只有一个区域，像两个粘在一起的球。
STATUS_BRONCHUS = 3      # 支气管阶段。特征：tracheo图像有两个有一定圆形度的部分。此后不再检测
    ...
    if status >= STATUS_SWITCH:
        ...
        if True == flag and STATUS_LARYNX == status:
            ...
            if STATUS_LARYNX == status:
                status = STATUS_TRACHEO
                print("=========== STATUS_TRACHEO region ==========")
            current_situation = get_tracheo_region(src_img, main_part_contour, last_situation)
            # 如果发生狭窄
            if current_situation.stenosis_info.stenosis_flag and current_situation.stenosis_info.restore_tracheo_area != 0:
                    stenosis_degree = 1 - current_situation.area / current_situation.stenosis_info.restore_tracheo_area
            # 没有发生狭窄
            else:
                stenosis_degree = 1

            print("[debug] stenosis area:  ", current_situation.area, \
            "  ; restore_stenosis_area:  ", current_situation.stenosis_info.restore_tracheo_area,\
            " ; degree: ", stenosis_degree)

            return True, stenosis_degree, current_situation


# @function get_tracheo_region: 得到 气道区域面积 与 还原的原气道区域的面积
#           方法：原图取主体部分，截断后range(-1023, 3167.5)->(-1020, -200)，以阈值-880进行阈值分割，得到气道部分
# @param    src_img:                传入的dcm文件
# @param    main_part_contour:      主体区域轮廓
# @param    last_situation:         上一帧气管情况
# @return   current_situation:      本帧气管情况
def get_tracheo_region(src_img, main_part_contour, last_situation): 
    # Step 1. 预处理
    # 1.1 取主体部分
    ...
    for contour in contours:
        ...
        if area / 3 < blur_area:
            tracheo_contours.append(contour)
    print("tracheo size: ", len(tracheo_contours))
    
    global status
    # Step 5: 状态机判断
    if status == STATUS_TRACHEO:
        # status是气管阶段    
        if len(tracheo_contours) > 1:
            # 在status为气管阶段时气道数量超过一个，有可能是食道或肺泡带来的干扰（不会是支气管），借助上一帧气管所在位置判定保留气管，删除干扰
            # 两帧重合度大于0.8的话判定为同一孔洞
            tracheo_contours = rule_out_esophagus(tracheo_contours, src_img, last_situation.contour)
            print("new tracheo contour size: ", len(tracheo_contours))
        
        # 去除食道、肺泡等的干扰后，若只有一个气孔，则判断是否有狭窄和是否进入转换状态
        if 1 == len(tracheo_contours):
            current_situation.contour = tracheo_contours[0]
            ...
            if [] != last_situation.contour:
                current_situation.stenosis_info.restore_tracheo_area = current_situation.area
                
                # 狭窄状态中
                if last_situation.stenosis_info.stenosis_flag:
                    current_situation.stenosis_info.restore_tracheo_area = last_situation.stenosis_info.restore_tracheo_area            
                    ...
                    if current_situation.circularity < 0.87 and current_situation.area < last_situation.area:
                        ...
                        if 2 == last_situation.stenosis_info.stenosis_count:
                            current_situation.stenosis_info.restore_tracheo_area = last_situation.stenosis_info.restore_tracheo_area
                            print("### 确认狭窄。还原区域面积： ", current_situation.stenosis_info.restore_tracheo_area)
                            current_situation.stenosis_info.stenosis_flag = True
                            current_situation.stenosis_start_area = last_situation.stenosis_info.restore_tracheo_area
    ...
    tracheo = np.zeros_like(src_img, np.uint8)
    for contour in tracheo_contours:
        cv2.fillPoly(tracheo, [contour], 255)  
    if tracheo_contours != []:
        cv2.imshow('tracheo', tracheo)
    
    return current_situation

def rule_out_esophagus(tracheo_contours, src_img, last_contour):
    new_tracheo_contours = []
    for contour in tracheo_contours:
        overlap = 0
        tmp_img = np.zeros_like(src_img, np.uint8)
        cv2.fillPoly(tmp_img, [contour], 255)
        ...
        if overlap > 0.8:
            new_tracheo_contours.append(contour)
    return new_tracheo_contours
